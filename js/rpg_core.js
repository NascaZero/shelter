...
Bitmap.prototype.bltImageOnTop = function(source, x, y, alpha) {
...
Bitmap.prototype.drawBlackOnTop = function() {
	var canvas = document.getElementById('TopCanvas');
	var context = canvas.getContext("2d");
...
Bitmap.prototype.changeTopAlpha = function(alpha) {
	var canvas = document.getElementById('TopCanvas');
	var context = canvas.getContext("2d");
	context.globalAlpha = alpha;
}

Bitmap.prototype.getTopAlpha = function() {
	var canvas = document.getElementById('TopCanvas');
	var context = canvas.getContext("2d");
...
Bitmap.prototype.drawCube = function() {
	var geometry = new THREE.CubeGeometry(10,10,10);
	var material = new THREE.MeshBasicMaterial({color: 0x00ff00});
	var cube = new THREE.Mesh(geometry, material);
	Graphics._scene.add(cube);
	cube.name = "cube";
	cube.rotation.x += 0.5;
	cube.rotation.y += 0.5;
	Graphics._renderer3d.render(Graphics._scene, Graphics._camera);
};

Bitmap.prototype.drawSphere = function(){
	if(Graphics._sphere == null){
		var loader = new THREE.TextureLoader();
		loader.load("img/titles1/btns.png",function(texture){
			//texture.matrixAutoUpdate = false;
			var geometry = new THREE.SphereGeometry(34,60,60);
			var material = new THREE.MeshBasicMaterial({map:texture,transparent:true});
			Graphics._sphere = new THREE.Mesh(geometry,material);
			Graphics._scene.add( Graphics._sphere);
			//sphere.position.set(0,0,0);
			//Graphics._sphere.position.y = -0.6;
			Graphics._sphere.position.y = -100;
			Graphics._sphere.position.x = -100;
			Graphics._sphere.rotation.x = 0.4;
...
Bitmap.prototype.changeSpherePosition = function(x,y){
	if(Graphics.isRendering){
		Graphics._sphere.position.y = y;
		Graphics._sphere.position.x = x;
		Graphics._renderer3d.render(Graphics._scene, Graphics._camera);
	}
}

Bitmap.prototype.setSphereIndex = function(index){
	if(Graphics.isRendering){
		switch(index){
			case 0:Graphics._sphere.rotation.y = 0.55;break;
			case 1:Graphics._sphere.rotation.y = 0;break;
			case 2:Graphics._sphere.rotation.y = -0.55;break;
		}
		Graphics._renderer3d.render(Graphics._scene, Graphics._camera);
	}
}

Bitmap.prototype.updateSphere = function(index){
	if(Graphics.isRendering){
		switch(index){
			case 0:{
				if(Graphics._sphere.rotation.y - 0.55 < 0.0000001 && Graphics._sphere.rotation.y - 0.55 > -0.0000001){
					Graphics._sphere.rotation.y = 0.55;
					return false;
				} else if(Graphics._sphere.rotation.y < 0.55){
					Graphics._sphere.rotation.y += 0.01;
					Graphics._renderer3d.render(Graphics._scene, Graphics._camera);
					return true;
				} else if(Graphics._sphere.rotation.y > 0.55){
					Graphics._sphere.rotation.y -= 0.01;
			...
			case 1:{
				if(Graphics._sphere.rotation.y < 0.0000001 && Graphics._sphere.rotation.y > -0.0000001){
					Graphics._sphere.rotation.y = 0;
					return false;
				} else if(Graphics._sphere.rotation.y < 0){
					Graphics._sphere.rotation.y += 0.01;
					Graphics._renderer3d.render(Graphics._scene, Graphics._camera);
					return true;
				} else if(Graphics._sphere.rotation.y > 0){
...
Bitmap.prototype.drawPlane = function() {
	threeFillText = function(ctx,height,text,y){
		if(text.contains("\n")){
			var temp = text.split("\n");
			for(var i = 0;i < temp.length;i ++){
				var lineGradient = ctx.createLinearGradient(0,0,512,0);
				lineGradient.addColorStop(0.3, 'rgba(255, 255, 255, 1)'); 
				lineGradient.addColorStop(1, 'rgba(255, 255, 255, 0)'); 
				ctx.fillStyle = lineGradient;
				ctx.fillText(temp[i],10,y);
				y += height;
				if(y >= 512){break;}
			}
		}
	}
	Graphics._clearCanvas3d();
	var plane = null;
...
Bitmap.prototype.update3DText = function(y){
	if(Graphics.isRendering){
		var canvas = document.getElementById('Text');
		var ctx = canvas.getContext("2d");
		var text = "CanvasGraphicsRenderer.prototype.render = function render(graphics) {\n        var renderer = this.renderer;\n        var context = renderer.context;\n        var worldAlpha = graphics.worldAlpha;\n        var transform = graphics.transform.worldTransform;\n        var resolution = renderer.resolution;\n\n        // if the tint has changed, set the graphics object to dirty.\n        if (this._prevTint !== this.tint) {\n            this.dirty = true;\n        }\n\n        context.setTransform(transform.a * resolution, transform.b * resolution, transform.c * resolution, transform.d * resolution, transform.tx * resolution, transform.ty * resolution);\n\n        if (graphics.dirty) {\n            this.updateGraphicsTint(graphics);\n            graphics.dirty = false;\n        }\n\n        renderer.setBlendMode(graphics.blendMode);\n\n        for (var i = 0; i < graphics.graphicsData.length; i++) {\n            var data = graphics.graphicsData[i];\n            var shape = data.shape;\n\n            var fillColor = data._fillTint;\n            var lineColor = data._lineTint;\n\n            context.lineWidth = data.lineWidth;\n\n            if (data.type === _const.SHAPES.POLY) {\n                context.beginPath();\n\n                this.renderPolygon(shape.points, shape.closed, context);\n\n                for (var j = 0; j < data.holes.length; j++) {\n                    this.renderPolygon(data.holes[j].points, true, context);\n                }\n\n                if (data.fill) {\n                    context.globalAlpha = data.fillAlpha * worldAlpha;\n                    context.fillStyle = '#' + ('00000' + (fillColor | 0).toString(16)).substr(-6);\n                    context.fill();\n                }\n                if (data.lineWidth) {\n                    context.globalAlpha = data.lineAlpha * worldAlpha;\n                    context.strokeStyle = '#' + ('00000' + (lineColor | 0).toString(16)).substr(-6);\n                    context.stroke();\n                }\n            } else if (data.type === _const.SHAPES.RECT) {\n                if (data.fillColor || data.fillColor === 0) {\n                    context.globalAlpha = data.fillAlpha * worldAlpha;\n                    context.fillStyle = '#' + ('00000' + (fillColor | 0).toString(16)).substr(-6);\n                    context.fillRect(shape.x, shape.y, shape.width, shape.height);\n                }\n                if (data.lineWidth) {\n                    context.globalAlpha = data.lineAlpha * worldAlpha;\n                    context.strokeStyle = '#' + ('00000' + (lineColor | 0).toString(16)).substr(-6);\n                    context.strokeRect(shape.x, shape.y, shape.width, shape.height);\n                }\n            } else if (data.type === _const.SHAPES.CIRC) {\n                // TODO - need to be Undefined!\n                context.beginPath();\n                context.arc(shape.x, shape.y, shape.radius, 0, 2 * Math.PI);\n                context.closePath();\n\n                if (data.fill) {\n                    context.globalAlpha = data.fillAlpha * worldAlpha;\n                    context.fillStyle = '#' + ('00000' + (fillColor | 0).toString(16)).substr(-6);\n                    context.fill();\n                }\n                if (data.lineWidth) {\n                    context.globalAlpha = data.lineAlpha * worldAlpha;\n                    context.strokeStyle = '#' + ('00000' + (lineColor | 0).toString(16)).substr(-6);\n                    context.stroke();\n                }\n            } else if (data.type === _const.SHAPES.ELIP) {\n                // ellipse code taken from: http://stackoverflow.com/questions/2172798/how-to-draw-an-oval-in-html5-canvas\n\n                var w = shape.width * 2;\n                var h = shape.height * 2;\n\n                var x = shape.x - w / 2;\n                var y = shape.y - h / 2;\n\n                context.beginPath();\n\n                var kappa = 0.5522848;\n                var ox = w / 2 * kappa; // control point offset horizontal\n                var oy = h / 2 * kappa; // control point offset vertical\n                var xe = x + w; // x-end\n                var ye = y + h; // y-end\n                var xm = x + w / 2; // x-middle\n                var ym = y + h / 2; // y-middle\n\n                context.moveTo(x, ym);\n                context.bezierCurveTo(x, ym - oy, xm - ox, y, xm, y);\n                context.bezierCurveTo(xm + ox, y, xe, ym - oy, xe, ym);\n                context.bezierCurveTo(xe, ym + oy, xm + ox, ye, xm, ye);\n                context.bezierCurveTo(xm - ox, ye, x, ym + oy, x, ym);\n\n                context.closePath();\n\n                if (data.fill) {\n                    context.globalAlpha = data.fillAlpha * worldAlpha;\n                    context.fillStyle = '#' + ('00000' + (fillColor | 0).toString(16)).substr(-6);\n                    context.fill();\n                }\n                if (data.lineWidth) {\n                    context.globalAlpha = data.lineAlpha * worldAlpha;\n                    context.strokeStyle = '#' + ('00000' + (lineColor | 0).toString(16)).substr(-6);\n                    context.stroke();\n                }\n            } else if (data.type === _const.SHAPES.RREC) {\n                var rx = shape.x;\n                var ry = shape.y;\n                var width = shape.width;\n                var height = shape.height;\n                var radius = shape.radius;\n\n                var maxRadius = Math.min(width, height) / 2 | 0;\n\n                radius = radius > maxRadius ? maxRadius : radius;\n\n                context.beginPath();\n                context.moveTo(rx, ry + radius);\n                context.lineTo(rx, ry + height - radius);\n                context.quadraticCurveTo(rx, ry + height, rx + radius, ry + height);\n                context.lineTo(rx + width - radius, ry + height);\n                context.quadraticCurveTo(rx + width, ry + height, rx + width, ry + height - radius);\n                context.lineTo(rx + width, ry + radius);\n                context.quadraticCurveTo(rx + width, ry, rx + width - radius, ry);\n                context.lineTo(rx + radius, ry);\n                context.quadraticCurveTo(rx, ry, rx, ry + radius);\n                context.closePath();\n\n                if (data.fillColor || data.fillColor === 0) {\n                    context.globalAlpha = data.fillAlpha * worldAlpha;\n                    context.fillStyle = '#' + ('00000' + (fillColor | 0).toString(16)).substr(-6);\n                    context.fill();\n                }\n\n                if (data.lineWidth) {\n                    context.globalAlpha = data.lineAlpha * worldAlpha;\n                    context.strokeStyle = '#' + ('00000' + (lineColor | 0).toString(16)).substr(-6);\n                    context.stroke();\n                }\n            }\n        }\n    };\n";
...
Bitmap.prototype.drawTips = function(text, x, y, maxWidth, lineHeight, align, maxHeight ,type ,opacity) {
    if (text !== undefined) {
        var tx = x;
        var ty = y + lineHeight - (lineHeight - this.fontSize * 0.7) / 2;
        var context = this._context;
        var alpha = opacity / 255.0;
        maxWidth = maxWidth || 0xffffffff;
        if (align === 'center') {
            tx += maxWidth / 2;
        }
        ...
        context.textAlign = align;
        context.textBaseline = 'alphabetic';
        context.globalAlpha = opacity / 255.0;
		if(text.contains("\n")){
			var temp = text.split("\n");
			for(var i = 0;i < temp.length;i ++){
				context.globalAlpha = alpha;
				this._drawTextBody4CodeBlocks(temp[i], tx, ty, maxWidth,maxHeight,type);
				ty = parseFloat(ty) + this.fontSize;
			}
		} else {
			//this._drawTextOutline(text, tx, ty, maxWidth);
			context.globalAlpha = alpha;
			this._drawTextBody4CodeBlocks(text, tx, ty, maxWidth,maxHeight, type);
		}
        context.restore();
        this._setDirty();
    }
...
Bitmap.prototype.drawText4CodeBlocks = function(text, x, y, maxWidth, lineHeight, align, maxHeight ,type) {
    if (text !== undefined) {
        var tx = x;
        var ty = y + lineHeight - (lineHeight - this.fontSize * 0.7) / 2;
        var context = this._context;
        ...
        context.textBaseline = 'alphabetic';
        context.globalAlpha = 1;
		var lineGradient = context.createLinearGradient(0,0,0,maxHeight);
		lineGradient.addColorStop(0.3, 'rgba(255, 255, 255, 1)'); 
		lineGradient.addColorStop(1, 'rgba(255, 255, 255, 0)'); 
		...
		if(text.contains("\n")){
			...
			for(var i = 0;i < temp.length;i ++){
				context.globalAlpha = alpha;
				this._drawTextBody4CodeBlocks(temp[i], tx, ty, maxWidth,maxHeight,type,i);
				ty = parseFloat(ty) + this.fontSize;
			}
		} else {
			//this._drawTextOutline(text, tx, ty, maxWidth);
			context.globalAlpha = alpha;
			this._drawTextBody4CodeBlocks(text, tx, ty, maxWidth,maxHeight, type,0);
		}
        context.restore();
        this._setDirty();
    }
...
Bitmap.prototype._drawTextBody4CodeBlocks = function(text, tx, ty, maxWidth , maxHeight , type , i) {
    var context = this._context;
	if(type == 1){
		//纵向渐变
		...
		var lineGradient = context.createLinearGradient(0,maxWidth,maxHeight,maxWidth);
		lineGradient.addColorStop(0.3, 'rgba(255, 255, 255, 1)'); 
		lineGradient.addColorStop(1, 'rgba(255, 255, 255, 0)'); 
		context.fillStyle = lineGradient;
		//context.fillStyle = "#F00";
		context.save();
		context.transform(0.9,0.07,0,0.5,10,0);
		context.beginPath();
		context.moveTo(0,0);
		...
		context.transform(0.85,-0.1,0,0.5,22,38);
		context.beginPath();
		context.moveTo(maxWidth,maxHeight);
		context.lineTo(0,maxHeight);
		context.lineTo(maxWidth,0);
		context.closePath();
		context.clip();
...
Graphics.initialize = function(width, height, type) {
	...
	this._duringLogo = true;
	this._firstTitle = false;
...
Graphics._createAllElements = function() {
	...
	this._createCanvas3d();
	this._createTopCanvas();
...
Graphics._updateAllElements = function() {
    ...
    this._updateCanvas();
	this._updateCanvas3d();
	this._updateTopCanvas();
    this._updateVideo();
...
Graphics._createCanvas3d = function() {
	this._scene = new THREE.Scene();
	this._camera = new THREE.PerspectiveCamera(75, 16/9, 1, 10000);
	this._camera.position.z = 50;
	this._renderer3d = new THREE.WebGLRenderer({alpha:true,
	antialias: true,
	precision:"highp",
	preserveDrawingBuffer:true
	});
	this._renderer3d.setSize(1280, 720);
	this._canvas3d = this._renderer3d.domElement;
	this._canvas3d.id = "Canvas3D";
    this._updateCanvas3d();
    document.body.appendChild(this._canvas3d);
};

Graphics._updateCanvas3d = function() {
    this._canvas3d.width = this._width;
    this._canvas3d.height = this._height;
    this._canvas3d.style.zIndex = 2;
    this._centerElement(this._canvas3d);
	this._renderer3d.render(this._scene,this._camera);
};

Graphics._changeCanvas3dOpacity = function(op){
	this._canvas3d.style.opacity = op;
}

Graphics._clearCanvas3d = function() {
	this._renderer3d.clear();
}


Graphics._createTopCanvas = function() {
    this._topCanvas = document.createElement('canvas');
    this._topCanvas.id = 'TopCanvas';
    this._updateTopCanvas();
    document.body.appendChild(this._topCanvas);
};


Graphics._updateTopCanvas = function() {
    this._topCanvas.width = this._width;
    this._topCanvas.height = this._height;
    this._topCanvas.style.zIndex = 9;
    this._centerElement(this._topCanvas);
};

Graphics._clearTopCanvas = function() {
    var context = this._topCanvas.getContext('2d');
...
Graphics._bltOnTop= function(source, x, y, alpha)	{
	var canvas = document.getElementById('TopCanvas');
	var context = canvas.getContext("2d");
	var image = new Image();
	image.src = source;
...
Graphics._changeTopAlpha = function(alpha) {
	var canvas = document.getElementById('TopCanvas');
	var context = canvas.getContext("2d");
	context.globalAlpha = alpha;
};

Graphics._getTopAlpha = function(alpha) {
	var canvas = document.getElementById('TopCanvas');
	var context = canvas.getContext("2d");
	return context.globalAlpha;
};
...
Graphics._changeDuringLogo = function(flag) {
	this._duringLogo = flag;
};

Graphics._changeFirstTitle = function(flag) {
	this._firstTitle = flag;
};

Graphics._fillTopCanvas = function() {
	var canvas = document.getElementById("TopCanvas");
...
Graphics._drawLoadingOnTop = function(i,j,p,r0){
	var a = [1.45,1.45];
	var b = [0.45,0.45];
	var ass = [1.550,1.551,1.554,1.559,1.564,1.570,1.580,1.640,
	1.700,1.764,1.878,1.992,2.106,2.220,2.550,2.600];

	var bss = [1.380,1.490,1.520,1.650,1.800,2.200,2.34];
	var canvas = document.getElementById("TopCanvas");
	ctx = canvas.getContext("2d");
	w = 1280,
	h = 720,
	...
	if(p <= 150){
		...
		if(p >= 40 && p < 60) re = 165;
		if(p >= 60 && p < 80) re = 245;
		if(p >= 80 && p < 100) re = 345;
		if(p >= 100) re = 345 + (p-99)/50.0 * 1000;
 		var grd = ctx.createRadialGradient(x,y,0,x,y,re);
		grd.addColorStop(1,"rgba(0,0,0,1)");
		grd.addColorStop(0,"rgba(0,0,0,0)");
		if(p >10) ctx.fillStyle=grd;
	...
	if(p >= 100 && i>=15 && j>= 14){
		...
		if(p <= 150){
			...
			ctx.fill();
			ctx.closePath();
			clearArc(x,y,(p - 120)/30 * (r - 16));
		} else {
			//when p bigger than 150 & less than 164
			//draw two out lines and clear the arc
			if(p - 150 <= 14){
				point = bss[(p-150)/2];
			
				ctx.lineWidth = 8;
				ctx.beginPath();
				ctx.arc(x,y,r,point*Math.PI,2.34*Math.PI);
				ctx.strokeStyle="white";
				ctx.stroke();
				ctx.closePath();
				
				ctx.beginPath();
				ctx.arc(x,y,r,(point-1.0)*Math.PI,1.34*Math.PI);
				ctx.stroke();
				ctx.closePath();
				...
				ctx.beginPath();
				ctx.moveTo(591,291);
				ctx.lineTo(591+234*(p-150)/14,291-150*(p-150)/14);
				ctx.stroke();
				ctx.closePath();
				
				ctx.beginPath();
				ctx.moveTo(685,432);
				ctx.lineTo(685-234*(p-150)/14,432+150*(p-150)/14);
				ctx.stroke();
				ctx.closePath();
			} else if(p - 164 <= 12){
				//when p bigger than 164
				//clear the lines
				ctx.lineWidth = 8;
				ctx.strokeStyle="white";
				ctx.beginPath();
				ctx.moveTo(825,141);
				ctx.lineTo(591+234*(p - 164)/12.0,291-150*(p - 164)/12.0);
				ctx.stroke();
				ctx.closePath();
				
				ctx.beginPath();
				ctx.moveTo(451,582);
				ctx.lineTo(685-234*(p - 164)/12.0,432+150*(p - 164)/12.0);
				ctx.stroke();
				ctx.closePath();
				
				Graphics._bltOnTop("img/titles1/GF_Logo.png",437,182,(p - 164)/10.0);
				
			} else if(p - 176 <= 6){
				Graphics._bltOnTop("img/titles1/GF_Logo.png",437,182,1);
				Graphics._bltOnTop("img/titles1/Logo_3.png",569,270,(p - 176)/6.0);
			} else if(p - 182 <= 12){
				Graphics._bltOnTop("img/titles1/GF_Logo.png",437,182,1);
				Graphics._bltOnTop("img/titles1/Logo_3.png",569,270,1);
				
				x1=424-10*(p - 182)/12.0;
				y1=261+10*(p - 182)/12.0;
				
				x2=644+10*(p - 182)/12.0;
				y2=270+10*(p - 182)/12.0;
				//起始
				Graphics._bltOnTop("img/titles1/Logo_1.png",x1,y1,(p - 182)/12.0);
				//避难所
				Graphics._bltOnTop("img/titles1/Logo_2.png",x2,y2,(p - 182)/12.0);
			} else {
				Graphics._bltOnTop("img/titles1/GF_Logo.png",437,182,1);
				Graphics._bltOnTop("img/titles1/Logo_3.png",569,270,1);
				//起始
				Graphics._bltOnTop("img/titles1/Logo_1.png",414,271,1);
				//避难所
				Graphics._bltOnTop("img/titles1/Logo_2.png",654,280,1);
			}
		...
		if(p >= 100){
			op = (130-p) / 130 * 1.0;
			style1 = 'rgba(255,255,255,'+ op +')';
			style2 = 'rgba(95,95,95,'+ op +')';
			style3 = 'rgba(0, 17, 102,'+ op +')';
			ctx.fillStyle = style1;
			ctx.shadowColor = style2;
			ctx.strokeStyle = style3;
			ctx.strokeText("100%",x,y + 20);
			ctx.fillText("100%",x,y + 20);
		} else {
			ctx.save();
			ctx.fillStyle = "white";
			ctx.shadowColor = 'rgba(95,95,95,1)';
			ctx.strokeStyle = "rgb(0, 17, 102)";
			ctx.strokeText(p+"%",x,y + 20);
		...
		as = parseFloat(a[0]).toFixed(3);
		ae = parseFloat(a[1]).toFixed(3);
		
		ctx.lineWidth = 8;
		ctx.beginPath();
		ctx.arc(x,y,r,as*Math.PI,ae*Math.PI);
		ctx.strokeStyle="white";
		ctx.stroke();
		ctx.closePath();
		
		
		bs = parseFloat(b[0]).toFixed(3);
		be = parseFloat(b[1]).toFixed(3);
		ctx.beginPath();
		ctx.arc(x,y,r,bs*Math.PI,be*Math.PI);
		ctx.stroke();
		ctx.closePath();
	}
