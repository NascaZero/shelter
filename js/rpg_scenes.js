...
Scene_Boot.prototype.start = function() {
    ...
    if (DataManager.isBattleTest()) {
		...
		Graphics._bltOnTop("img/titles1/logo.png",0,0,0);
		change = setInterval(function(){changeAlpha()}, 100);
		function changeAlpha(){
			alpha = Graphics._getTopAlpha();
			alpha += 0.05;
			if(alpha >= 1.0){
				clearInterval(change);
				Graphics._changeDuringLogo(false);
				SceneManager.goto(Scene_Title);
				Window_TitleCommand.initCommandPosition();
			}
			Graphics._clearTopCanvas();
			Graphics._bltOnTop("img/titles1/logo.png",0,0,alpha);
		}
...
Scene_Title.prototype.create = function() {
	...
	this._isBlacking = 0;
	this._loadingCircleI = 0;
	this._loadingCircleJ = 0;
	this._loadingCircleP = 0;
	this._loadingR0 = 0;
	this._updateTimes = 0;
	this.duringLogo = true;
	this.duringCommand = false;
};
...
Scene_Title.prototype.createBlack = function() {
	this._blackSprite = new Sprite(new Bitmap(1,1));
	this.addChild(this._blackSprite);
	this._blackSprite.bitmap.drawBlackOnTop();
	this._isBlacking = 1;
}

Scene_Title.prototype.update = function() {
	if(this._isBlacking == 0 ){
		Graphics._clearTopCanvas();
		...
		this.createSphereCommand();
		
		this.createBlack();
	} else if(this._isBlacking == 1){
		if(this._blackSprite.bitmap.getTopAlpha() > 0.05){
			alpha = this._blackSprite.bitmap.getTopAlpha();
			alpha -= 0.01;
			this._blackSprite.bitmap.changeTopAlpha(alpha);
		} else {
			this._isBlacking = 2;
			Graphics._clearTopCanvas();
			...
			this.setSphereCommandIndex();
		}
	} else if(this._isBlacking == 2){
		if(Graphics._firstTitle == true){
			if(this._loadingCircleP>20)
				this.updateTexts();
			if(this._updateTimes % 4 == 0){
				Graphics._clearTopCanvas();
				Graphics._drawLoadingOnTop(this._loadingCircleI,this._loadingCircleJ,this._loadingCircleP,this._loadingR0);
				if(this._loadingCircleP<200){
					this._loadingCircleP += 2;
					if(!(this._loadingCircleP > 100 && this._loadingCircleI >= 15 && this._loadingCircleJ >= 14)){
						if(this._loadingCircleI == 15){
							if(this._loadingCircleJ == 14){
								this._loadingCircleJ++;
							}else{
								this._loadingCircleJ++;
								this._loadingCircleJ++;
							}
						...
						if(this._loadingCircleI<15){
							this._loadingCircleI++;
						} else if(this._loadingCircleJ==15 && this._loadingCircleI==15){
							this._loadingCircleI=0;
							this._loadingCircleJ=0;
						}
					}
					if(this._loadingR0 < 89) this._loadingR0+=1.5;
				} else {
					this.correctSphereCommand();
					//Graphics._drawLoadingOnTop(this._loadingCircleI,this._loadingCircleJ,199,this._loadingR0);
					Graphics._firstTitle = false;
				}
			}
			this._updateTimes++;
		} else {
			this.correctSphereCommand();
			this.updateSphereCommand();
			this.updateTexts();
			this.updateTips();
		}
	}
...
Scene_Title.prototype.updateTexts = function() {
	//Update codeblocks
	if(this._codeBSY < -2500){
		this._codeBSY = 500;
	} else {
		this._codeBSY -= 5;
		this._codeBlockFlag1 ++;
	}
	this.updateCodeBlocks(this._codeBSY);
	
	
	
	
	/* //update and wait 
	if(this._codeBlockFlag1 < 100){
		if(this._codeBSY < -800){
			this._codeBSY = 0;
		} else {
			this._codeBSY -= 5;
			this._codeBlockFlag1 ++;
		...
		this.updateCodeBlocks(this._codeBSY);
	} else if(this._codeBlockFlag1 >= 100 && this._codeBlockFlag1 < 250){
		this._codeBlockFlag1 ++;
	} else {
		this._codeBlockFlag1 = 0;
	} */
}


Scene_Title.prototype.updateTips = function() {
	//Update tips
	if(this._opacityFlag){
		this._tipsOpacity -= 5;
		if(this._tipsOpacity <= 0){
			this._opacityFlag = false;
		}
	} else {
		this._tipsOpacity += 5;
		if(this._tipsOpacity >= 255){
			this._opacityFlag = true;
		}
	}
	
	switch(this._commandIndex){
		case 0:this.drawTips("Press the start button to\ncontinue your game",this._tipsOpacity);break;
		case 1:this.drawTips("Press the start button to\nstart a new game",this._tipsOpacity);break;
		case 2:this.drawTips("Press the start button to\nconfig your game",this._tipsOpacity);break;
	}
}

Scene_Title.prototype.setSphereCommandIndex = function() {
	this._codeBlockSprite1.bitmap.setSphereIndex(this._commandIndex);
}

Scene_Title.prototype.updateSphereCommand = function() {
	if(!this.duringCommand){
		if(Input.isTriggered('left')){
			if(this._commandIndex > 0){
				this._commandIndex --;
				this.duringCommand = this._codeBlockSprite1.bitmap.updateSphere(this._commandIndex);
			}
		...
		if(Input.isTriggered('ok') && Graphics.isRendering == true){
			Graphics.isRendering = false;
			switch(this._commandIndex){
				case 1:this.commandNewGame();break;//New Game
				case 0:this.commandContinue();break;//Continue
				case 2:this.commandOptions();break;//Exit
			}
		}
	} else {
		this.duringCommand = this._codeBlockSprite1.bitmap.updateSphere(this._commandIndex);
...
Scene_Title.prototype.createTitleText = function() {
	this._titleText = new Sprite(new Bitmap(450,356));
	/* this._titleText.bitmap.bltImageOnTop("img/titles1/GF_Logo.png",437,182,1);
	this._titleText.bitmap.bltImageOnTop("img/titles1/Logo_3.png",569,270,1);
	this._titleText.bitmap.bltImageOnTop("img/titles1/Logo_1.png",414,271,1);
	this._titleText.bitmap.bltImageOnTop("img/titles1/Logo_2.png",654,280,1); */
	this._titleText.bitmap.bltImageOnTop("img/titles1/Title_Text_Chapter.1.png",(1280 - 450) / 2,(720 - 356) / 2,1);
	this._titleText.x = 100;
...
Scene_Title.prototype.createCodeBlocks = function() {
	//code addition
	this._codeBlockSprite1 = new Sprite(new Bitmap(400, 400));
	
	this._codeBlockFlag1 = 0;
	this._codeBlockSprite1.x = 810;
	this._codeBlockSprite1.y = 0;
	this._codeBlockText1 = "import java.awt.image.BufferedImage;\nimport java.awt.image.PixelGrabber;\nimport java.io.IOException;\nimport java.nio.ByteBuffer;\n\nimport javax.imageio.ImageIO;\nimport com.sun.opengl.util.BufferUtil;\n\npublic class TextureReader {\n    public static Texture readTexture(String filename) throws IOException {\n        return readTexture(filename,true);\n    }\n\n    public static Texture readTexture(String filename, boolean storeAlphaChannel) throws IOException {\n        BufferedImage bufferedImage;\n        if (filename.endsWith(\".bmp\")) {\n            //bufferedImage = BitmapLoader.loadBitmap(filename);\n            bufferedImage = null;\n        } else {\n            bufferedImage = readImage(filename);\n        }\n        return readPixels(bufferedImage, storeAlphaChannel);\n    }\n\n    public static Texture readTexture(BufferedImage bufferedImage, boolean storeAlphaChannel) throws IOException {\n        return readPixels(bufferedImage, storeAlphaChannel);\n    }\n\n    /**\n     * 读取图片到 BufferedImage\n     * @param resourceName\n     * @return\n     * @throws IOException\n     */\n    private static BufferedImage readImage(String resourceName) throws IOException {\n        return ImageIO.read(TextureReader.class.getResourceAsStream(resourceName));\n    }\n\n    private static Texture readPixels(BufferedImage img, boolean storeAlphaChannel) {\n        int[] packedPixels = new int[img.getWidth() * img.getHeight()];\n\n        /**\n         */\n        PixelGrabber pixelgrabber = new PixelGrabber(img, 0, 0, img.getWidth(), img.getHeight(), packedPixels, 0, img.getWidth());\n        try {\n            pixelgrabber.grabPixels();\n        } catch (InterruptedException e) {\n            throw new RuntimeException();\n        }\n\n        int bytesPerPixel = storeAlphaChannel ? 4 : 3;\n        ByteBuffer unpackedPixels = BufferUtil.newByteBuffer(packedPixels.length * bytesPerPixel);\n\n\n        for (int row = 0; row < img.getHeight(); row++) {\n            for (int col = 0; col < img.getWidth(); col++) {\n                int packedPixel = packedPixels[row * img.getWidth() + col];\n                unpackedPixels.put((byte) ((packedPixel >> 16) & 0xFF));\n                unpackedPixels.put((byte) ((packedPixel >> 8) & 0xFF));\n                unpackedPixels.put((byte) ((packedPixel >> 0) & 0xFF));\n                if (storeAlphaChannel) {\n                    unpackedPixels.put((byte) ((packedPixel >> 24) & 0xFF));\n                }\n            }\n        }\n\n        unpackedPixels.flip();\n        return new Texture(unpackedPixels, img.getWidth(), img.getHeight());\n    }\n\n    /**\n     */\n    public static class Texture {\n        private int height;\n\n        public Texture(ByteBuffer pixels, int width, int height) {\n            this.height = height;\n            this.pixels = pixels;\n            this.width = width;\n        }\n\n        public int getHeight() {\n            return height;\n        }\n\n        public ByteBuffer getPixels() {\n            return pixels;\n        }\n\n        public int getWidth() {\n            return width;\n        }\n    }\n}\n";
	this._codeBSY = 0;
	this.addChild(this._codeBlockSprite1);
	this._codeBlockSprite2 = new Sprite(new Bitmap(300,200));
	this._codeBlockSprite2.bitmap.drawPlane();
	this.addChild(this._codeBlockSprite2);
	
	
	this._codeBlockSprite3 = new Sprite(new Bitmap(137, 224));
	this._codeBlockSprite3.x = 440;
	this._codeBlockSprite3.y = 287;
	this._codeBlockText3 = "import java.awt.image.BufferedImage;\nimport java.awt.image.PixelGrabber;\nimport java.io.IOException;\nimport java.nio.ByteBuffer;\n\nimport javax.imageio.ImageIO;\nimport com.sun.opengl.util.BufferUtil;\n\npublic class TextureReader {\n    public static Texture readTexture(String filename) throws IOException {\n        return readTexture(filename,true);\n    }\n\n    public static Texture readTexture(String filename, boolean storeAlphaChannel) throws IOException {\n        BufferedImage bufferedImage;\n        if (filename.endsWith(\".bmp\")) {\n            //bufferedImage = BitmapLoader.loadBitmap(filename);\n            bufferedImage = null;\n        } else {\n            bufferedImage = readImage(filename);\n        }\n        return readPixels(bufferedImage, storeAlphaChannel);\n    }\n\n    public static Texture readTexture(BufferedImage bufferedImage, boolean storeAlphaChannel) throws IOException {\n        return readPixels(bufferedImage, storeAlphaChannel);\n    }\n\n    /**\n     * 读取图片到 BufferedImage\n     * @param resourceName\n     * @return\n     * @throws IOException\n     */\n    private static BufferedImage readImage(String resourceName) throws IOException {\n        return ImageIO.read(TextureReader.class.getResourceAsStream(resourceName));\n    }\n\n    private static Texture readPixels(BufferedImage img, boolean storeAlphaChannel) {\n        int[] packedPixels = new int[img.getWidth() * img.getHeight()];\n\n        /**\n         */\n        PixelGrabber pixelgrabber = new PixelGrabber(img, 0, 0, img.getWidth(), img.getHeight(), packedPixels, 0, img.getWidth());\n        try {\n            pixelgrabber.grabPixels();\n        } catch (InterruptedException e) {\n            throw new RuntimeException();\n        }\n\n        int bytesPerPixel = storeAlphaChannel ? 4 : 3;\n        ByteBuffer unpackedPixels = BufferUtil.newByteBuffer(packedPixels.length * bytesPerPixel);\n\n\n        for (int row = 0; row < img.getHeight(); row++) {\n            for (int col = 0; col < img.getWidth(); col++) {\n                int packedPixel = packedPixels[row * img.getWidth() + col];\n                unpackedPixels.put((byte) ((packedPixel >> 16) & 0xFF));\n                unpackedPixels.put((byte) ((packedPixel >> 8) & 0xFF));\n                unpackedPixels.put((byte) ((packedPixel >> 0) & 0xFF));\n                if (storeAlphaChannel) {\n                    unpackedPixels.put((byte) ((packedPixel >> 24) & 0xFF));\n                }\n            }\n        }\n\n        unpackedPixels.flip();\n        return new Texture(unpackedPixels, img.getWidth(), img.getHeight());\n    }\n\n    /**\n     */\n    public static class Texture {\n        private int height;\n\n        public Texture(ByteBuffer pixels, int width, int height) {\n            this.height = height;\n            this.pixels = pixels;\n            this.width = width;\n        }\n\n        public int getHeight() {\n            return height;\n        }\n\n        public ByteBuffer getPixels() {\n            return pixels;\n        }\n\n        public int getWidth() {\n            return width;\n        }\n    }\n}\n";
	this.addChild(this._codeBlockSprite3);
	
	this.drawCodeBlocks();
	
	
	this._tips = new Sprite(new Bitmap(150,40));
	this.addChild(this._tips);
	this._tips.x = 565;
	this._tips.y = 650;
	this._tipsOpacity = 0;
	this._commandIndex = 1;
	this.drawTips("Press the start button to\nstart a new game",this._tipsOpacity);
}

Scene_Title.prototype.createSphereCommand = function(){
	this._codeBlockSprite2.bitmap.drawSphere();
	//this._codeBlockSprite2.bitmap.changeSpherePosition(-100,-100);
}

Scene_Title.prototype.correctSphereCommand = function(){
	this._codeBlockSprite2.bitmap.changeSpherePosition(0,-4.5);
}

Scene_Title.prototype.drawTips = function(text,opacity) {
    var x = 0;
	var y = 0;
    var maxWidth = 150;
    this._tips.bitmap.fontSize = 12;
	this._tips.bitmap.clear();
    this._tips.bitmap.drawTips(text, x, y, maxWidth, 10, 'center', 40, 1, opacity);
};

Scene_Title.prototype.drawCodeBlocks = function() {
    var x = 0;
	var y = 0;
    var maxWidth1 =  this._codeBlockSprite1.bitmap.width;
	var maxHeight1 = this._codeBlockSprite1.bitmap.height;
    this._codeBlockSprite1.bitmap.fontSize = 10;
    this._codeBlockSprite1.bitmap.drawText4CodeBlocks(this._codeBlockText1, x, y, maxWidth1, 10, 'left', maxHeight1, 1);
	
	var maxWidth3 =  this._codeBlockSprite3.bitmap.width;
	var maxHeight3 = this._codeBlockSprite3.bitmap.height;
	this._codeBlockSprite3.bitmap.fontSize = 10;
	this._codeBlockSprite3.bitmap.drawText4CodeBlocks(this._codeBlockText3, x, y, maxWidth3, 10, 'left', maxHeight3, 1);
};



Scene_Title.prototype.updateCodeBlocks = function(y) {
	var x = 0;
	var maxWidth1 =  this._codeBlockSprite1.bitmap.width;
	var maxHeight1 = this._codeBlockSprite1.bitmap.height;
	this._codeBlockSprite1.bitmap.clear();
	this._codeBlockSprite1.bitmap.drawText4CodeBlocks(this._codeBlockText1, x, y+400, maxWidth1, 10, 'left', maxHeight1, 1);
	
	this._codeBlockSprite2.bitmap.clear();
	this._codeBlockSprite2.bitmap.update3DText(y);
	
	var maxWidth3 =  this._codeBlockSprite3.bitmap.width;
	var maxHeight3 = this._codeBlockSprite3.bitmap.height;
	this._codeBlockSprite3.bitmap.clear();
	this._codeBlockSprite3.bitmap.drawText4CodeBlocks(this._codeBlockText3, x, y+800, maxWidth3, 10, 'left', maxHeight3, 1);
}
	

Scene_Title.prototype.drawGameTitle = function() {
    var x = 20;
	var y = Graphics.height - 70;
